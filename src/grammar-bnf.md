...
We then might recognize some familiar patterns: `<article> <noun>` captures the determination or quantification of a noun, so might mentally group these symbols (into what grammaticists call *nominal phrases* or *noun phrases*) and represent them by a new symbol.
...
which we might recognize the canonical structure of a well-formed sentence: *a thing does a thing.*
...
Putting these steps in reverse order, and starting with a single symbol `<sentence>` we get something that looks like a *proof* or *evidence* that `the dog jumped over the moon` is a valid sentence.\
...
In other words, a representation of our congnitive process.
And if we squint, we can see something that looks a bit like the parse tree in the introduction to this chapter.
...
> **Remark.** We typically will not state outright what the underlying symbols of a grammar are.
...
For example, `the dog jumped` is a sentence whereas `the <noun> jumped` is a sentential form.
...
> A **BNF specification** is a collection of production rules, together with a designated the **starting symbol**.

We read a production rule as saying

> The non-terminal symbol on the left-hand side can be replaced with the sentential form on the right hand side.
...
A correct derivation (according to the above definition) would look like
```
<sentence>!
<noun-phrase>!     <verb-phrase>
<noun-phrase>      <verb>  <prep-phrase>!
...
For emphasis I appended an exclamation point to the the non-terminal symbol which is replaced at teach step.
...
## A Programming Languages Example
...
Again, note that we're not interested in white space when we consider whether or not a sentence is recognized by a grammar.
...
> **Definition.** A **parse tree** for a sentence `S` in a grammar `G` is a (ordered) tree `T` with the following properties.
>
> * Every leaf has a terminal symbol. and
> * Every non-leaf node has a nonterminal symbol.
> * If a node `n` with has children `[t1, t2, ..., tk]` then
>   ```
>	val(n) ::= root(t1) root(t2) ... root(tk)
>	```
>	is a production rule in the grammar.
> * The leaves (in order) form the sentence `S`.
